          ...
          <tbody>
            {records.length === 0 ? (
              ...
              <tr key={record.id}>
                ...
                <td>{new Date(record.date).toLocaleDateString('en-US', {numberingSystem: 'latn'})}</td>
                <td>
                  <span className="badge badge-success">
                    {new Date(record.checkIn).toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit', numberingSystem: 'latn'})}
                  </span>
                </td>
                <td>
                  {record.checkOut ? (
                    <span className="badge badge-warning">
                      {new Date(record.checkOut).toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit', numberingSystem: 'latn'})}
                    </span>
                  ) : '-'}
                </td>
        ...
        <div className="modal-backdrop">
          <div className="modal-content glass-card">
            ...
            <p style={{ marginBottom: '1rem', color: 'var(--text-muted)' }}>
              يوم: {new Date(editingRecord.date).toLocaleDateString('en-US', {numberingSystem: 'latn'})} - الموظف: {editingRecord.employeeName}
            </p>
