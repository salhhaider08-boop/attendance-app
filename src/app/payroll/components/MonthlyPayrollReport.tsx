        ...
        <tbody>
          {payrollDataList.map((data, index) => (
            <tr key={data.employeeId}>
              ...
              <td>{data.basicSalary.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})}</td>
              <td style={{ fontSize: '0.9rem' }}>
                <span style={{ color: 'var(--success)' }}>{data.totalOfficialHours.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})} س</span> / <span style={{ color: 'var(--danger)' }}>{(data.totalOvertimeHours + data.totalFridayHours).toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})} س</span>
              </td>
              <td>{data.allowanceEarned.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})}</td>
              <td style={{ color: data.previousDues > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>
                {data.previousDues > 0 ? `+ ${data.previousDues.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})}` : '-'}
              </td>
              <td style={{ color: data.advances > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>
                {data.advances > 0 ? `- ${data.advances.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})}` : '-'}
              </td>
              <td style={{ fontWeight: 'bold', color: 'var(--primary)', fontSize: '1.1rem' }}>
                {data.finalSalary.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})} د.ع
              </td>
            </tr>
          ))}
          ...
          <tfoot>
            <tr style={{ background: 'rgba(79, 70, 229, 0.1)', borderTop: '2px solid var(--primary)' }}>
              ...
              <td style={{ fontWeight: 'bold', fontSize: '1.4rem', color: 'var(--primary)' }}>
                {totalSalaries.toLocaleString('en-US', {maximumFractionDigits: 0, numberingSystem: 'latn'})} د.ع
              </td>
            </tr>
          </tfoot>
        )}
